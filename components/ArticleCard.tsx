...
export default function ArticleCard({ article, showLink = true, id }: ArticleCardProps) {
  ...
  const articleContent = (
    <>
      ...
      {!article.gettyWidgetConfig && !gettyImageHtml && !hasGettyImageInContent && article.image && article.image.trim() !== '' && (
      ...
      <div className="px-4 md:px-6 lg:px-8">
        <h1 className={`text-black font-bold text-3xl md:text-4xl lg:text-5xl mb-6 md:mb-8 ${!showLink && article.gettyWidgetConfig ? 'mt-0' : !showLink ? 'mt-0' : 'mt-4'} md:mt-6 leading-tight text-balance`}>
          {article.title}
        </h1>
